...
impl<'a, T: General> VecView<'a, T>
{
    #[inline(always)]
    pub fn to_vec( &self ) -> Vec<T> {
        let mut x = Vec::<T>::with_capacity(self.size());
...
{
    #[inline(always)]
    pub fn new( m: usize, n: usize, data: &'a [T] ) -> Self {
    ...
    #[inline(always)]
    pub fn to_arr2( &self ) -> Mat<T> {
        let mut data = Vec::<T>::with_capacity(self.size());
        for val in self.data.iter() {
            data.push(*val);
        }
        Mat::<T>::from_vec(data, self.nrow(), self.ncol())
    }
}
...
{
    #[inline(always)]
    pub fn new( m: usize, n: usize, data: &'a mut [T] ) -> Self {
        Self { dim0: n, dim1: m, data }
    }
...
pub trait GVecMut<T: General>: GVec<T>
{
    ...
    #[inline(always)]
    fn get_elems<VT: GVec<T>>( &mut self, x: &VT, sel: &[usize] ) {
...
pub trait GMat<T: General>: GVec<T>
{
    ...
    #[inline(always)]
    fn idx_2d_to_1d( &self, i: usize, j: usize ) -> usize {
        i+j*self.nrow()
    }
