...
fn main() {
    ...
    let mut model = PlrGls::new_feasible(y, X, T).unwrap();
    model.estimate();
    ...
    let pr1: RMatPrinter = RMatPrinter::new(print_width, print_prec);
    pr1.print("V_hat", &model.V);
    let cor = cov_to_cor(&model.V);
    pr1.print("V_hat converted to correlation", &cor);
    println!();
